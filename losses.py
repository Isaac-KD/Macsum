...
def IMSELoss(y_true: torch.Tensor, y_low: torch.Tensor,
             y_up: torch.Tensor) -> torch.Tensor:
    """
    Interval Mean Squared Error (IMSE).
    Extended quadratic distance as defined in Eq. 7 of the research paper.
    """
    return torch.mean((y_true - y_low)**2 + (y_true - y_up)**2)


def PinballLoss(y_true: torch.Tensor, y_low: torch.Tensor,
                y_up: torch.Tensor, beta: float = 0.05) -> torch.Tensor:
    """
...
def SPILoss(y_true: torch.Tensor, y_low: torch.Tensor, y_up: torch.Tensor,
            beta: float = 0.05, k_sigmoid: float = 50.0) -> torch.Tensor:
    """
