...
class MacsumCore(nn.Module):
    ...
    def __init__(self, N: int, phi_init=None, device="cpu"):
        ...
        if phi_init is None:
            phi_init_tensor = torch.randn(N, device=device, dtype=torch.float32)
        elif isinstance(phi_init, np.ndarray):
            phi_init_tensor = torch.tensor(phi_init, device=device, dtype=torch.float32)
        else: 
            phi_init_tensor = phi_init.clone().detach().to(device=device, dtype=torch.float32)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if n_features != self.N:
            raise ValueError(f"Input feature dimension must be {self.N}, got {n_features}")
        
        # Calculate permutations (sorting does not require gradients)
        with torch.no_grad():
            perm_decreasing = torch.argsort(self._phi, descending=True).long()

        # Split phi into positive and negative parts while keeping autograd connection
        ...
        y_upper = torch.matmul(diff_max, phi_plus_sorted) + torch.matmul(diff_min, phi_minus_sorted)
        y_lower = torch.matmul(diff_min, phi_plus_sorted) + torch.matmul(diff_max, phi_minus_sorted)
