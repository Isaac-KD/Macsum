...
def run_sanity_check():
    ...
    # 1. Data Generation
    print(f"{INFO}[INFO] Generating synthetic data (100 samples, 5 features)...{RESET}")
    ...
    try:
        ...
        model.train(X_train, y_train, loss_fn=IMSELoss, epochs=3, batch_size=32)
        print(f"{GREEN}[SUCCESS] Training completed in {time.time() - start_time:.2f} seconds.{RESET}")
    except Exception as e:
    ...
    if len(history.get('train_loss', [])) == 3:
        print(f"{GREEN}[SUCCESS] History correctly recorded metrics for all 3 epochs.{RESET}")
    else:
        print(f"{RED}[ERROR] History does not contain the expected number of epochs.{RESET}")
    ...
    try:
        ...
        print(f"{GREEN}[SUCCESS] Predictions generated and validated successfully.{RESET}")
    ...
    print(f"{GREEN}\n[SUCCESS] All modules are communicating correctly. Codebase validated.{RESET}")
